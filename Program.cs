...
var cestudentsList = studentsList
    .Where(s => s.Branch == "CE")
    .ToList();

//2. studentsList having CPI > 8.
var highCPIstudentsList = studentsList
    .Where(s => s.CPI > 8)
    .ToList();

//3. studentsList older than 20.
var studentsListOlderThan20 = studentsList
    .Where(s => s.Age > 20)
    .ToList();

//4. studentsList in Semester 3.
studentsListOlderThan20 = studentsList
    .Where(s => s.Sem == 3)
    .ToList();

//5. CPI between 7 and 9.
var studentsListWithCPIBetween7And9 = studentsList
    .Where(s => s.CPI >= 7 && s.CPI <= 9)
    .ToList();

//6. Name starting with 'A'.
var studentsListWithNameStartingWithA = studentsList
    .Where(s => s.Name.StartsWith("A"))
    .ToList();

//7. Branch = IT AND Sem = 3.
var itstudentsListInSem3 = studentsList
    .Where(s => s.Branch == "IT" && s.Sem == 3)
    .ToList();

//8. Age < 20 OR CPI > 8.
var youngOrHighCPIstudentsList = studentsList
    .Where(s => s.Age < 20 || s.CPI > 8)
    .ToList();

//9. Names containing 'a'.
var studentsListWithNameContainingA = studentsList
    .Where(s => s.Name.Contains("a"))
    .ToList();

//10. studentsList NOT in CE.
var nonCEstudentsList = studentsList
    .Where(s => s.Branch != "CE")
    .ToList();

//11. Sem in {1,3,5}.
var studentsListInOddSemesters = studentsList
    .Where(s => new[] { 1, 3, 5 }.Contains(s.Sem))
    .ToList();

//12.studentsList whose CPI is a whole number.
studentsListWithNameStartingWithA = studentsList
    .Where(s => s.CPI % 1 == 0)
    .ToList();

//13. studentsList with even Roll No.
var studentsListWithEvenRollNo = studentsList
    .Where(s => s.Rno % 2 == 0)
    .ToList();

//14. studentsList whose age is between 18 and 21.
var studentsListAged18To21 = studentsList
    .Where(s => s.Age >= 18 && s.Age <= 21)
    .ToList();

//15. studentsList having name length > 4.
var studentsListWithLongNames = studentsList
...
var distinctBranches = studentsList
    .Select(s => s.Branch)
    .Distinct()
    .ToList();

//25. Convert student to “DTO” format (Rno, Name).
var studentDTOs = studentsList
...
var namesSortedAlphabetically = studentsList
    .OrderBy(s => s.Name)
    .ToList();

//27.Sort by CPI descending.
var studentsListSortedByCPIDesc = studentsList
    .OrderByDescending(s => s.CPI)
    .ToList();

//28. Sort by Sem, then Name.
var studentsListSortedBySemThenName = studentsList
    .OrderBy(s => s.Sem)
    .ThenBy(s => s.Name)
    .ToList();

//29.Sort by Age, then CPI desc.
var studentsListSortedByAgeThenCPIDesc = studentsList
    .OrderBy(s => s.Age)
    .ThenByDescending(s => s.CPI)
    .ToList();

//30. Sort by Branch.
var studentsListSortedByBranch = studentsList
 .OrderBy(s => s.Branch)
 .ToList();

//31. Sort by Name length.
var studentsListSortedByNameLength = studentsList
 .OrderBy(s => s.Name.Length)
 .ToList();

//32. Sort by Sem DESC.
var studentsListSortedBySemDesc = studentsList
 .OrderByDescending(s => s.Sem)
 .ToList();

//33. Sort by CPI then Age.
var studentsListSortedByCPIThenAge = studentsList
 .OrderBy(s => s.CPI)
 .ThenBy(s => s.Age)
 .ToList();

//34. Sort by Rno descending.
studentsListSortedByNameLength = studentsList
    .OrderByDescending(s => s.Rno)
    .ToList();

//35. Sort by Branch then Sem.
var studentsListSortedByBranchThenSem = studentsList
 .OrderBy(s => s.Branch)
 .ThenBy(s => s.Sem)
 .ToList();
...
var totalstudentsList = studentsList.Count();

//37.Count CE studentsList.
var totalCEstudentsList = studentsList
    .Count(s => s.Branch == "CE");
